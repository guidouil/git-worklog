# git-worklog — Documentation française
...
Le français est la langue de rapport par défaut, et l’anglais est disponible avec `--locale en`. Si l’environnement Node ne contient pas les données de locale française d’`Intl`/ICU, le rendu bascule automatiquement en anglais plutôt que d’échouer.
